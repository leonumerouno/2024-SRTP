from Literal import Literal
from TopK import TopK
import re


class Compare(object):
    def __init__(self):
        self.literal = Literal()
        self.other_compare = TopK()
        self.key_to_function = {
            '出生日期/出生年月': 'date',
            '逝世日期': 'date',
            '血型': 'blood',
            '年龄/年': 'age',
            '身高/高': 'height',
            '身高': 'height',
            '体重/重': 'weight',
            '性别': 'gender',
            '星座': 'constellation',
            '外文名/英语名': 'special_name'
        }

    def match_entity(self, sentence):
        pattern = re.compile(r'(.*)的(.*)是(.*)')
        match = pattern.match(sentence)
        if match:
            subject = match.group(1).strip()
            key = match.group(2).strip()
            value = match.group(3).strip()
            return subject, key, value
        else:
            return None

    def analysis_transformer(self, sentence_list, point_list):
        max_point = max(point_list)
        if max_point < 0.7:
            return ""
        else:
            # 第一步：将sentence_transformer的句子解析出来
            max_index = point_list.index(max_point)
            match_sentence = sentence_list[max_index]
            subject, key, value = self.match_entity(match_sentence)
            return subject, key, value

    def find_func(self, key):
        func_name = self.key_to_function.get(key, "others")
        func = getattr(self.literal, func_name, "others")
        return func_name, func

    def common(self, func, sentence, value):
        # 第三步：调用函数，从裂解后的最小分句提取出value
        entity_value = func(sentence)
        print(f"实体的值为: {entity_value}")

        if entity_value == value:
            return 1
        else:
            return 0

    def date(self, func, sentence, value):
        # 第三步中"date"的特殊处理
        result = True
        date_value = func(sentence)
        print(f"日期的值为: {date_value}")
        year_bool = True
        month_bool = True
        day_bool = True
        year_, month_, day_ = date_value.split('-')
        year, month, day = value.split('-')

        if year_ == "0000":
            year_bool = False
        elif month_ == "00":
            month_bool = False
        elif day_ == "00":
            day_bool = False

        if year_bool and month_bool and day_bool:
            if date_value != value:
                result = False
        elif year_bool and month_bool:
            if (year != year_ and year != "0000") or (month != month_ and month != "00"):
                result = False
        elif month_bool and day_bool:
            if (month != month_ and month != "00") or (day != day_ and day != "00"):
                result = False
        elif year_bool and day_bool:
            if (year != year_ and year != "0000") or (day != day_ and day != "00"):
                result = False
        elif year_bool:
            if year != year_ and year != "0000":
                result = False
        elif month_bool:
            if month != month_ and month != "00":
                result = False
        elif day_bool:
            if day != day_ and day != "00":
                result = False

        if result:
            return 1
        else:
            return 0

    def others(self, ecs, subject, sentence):
        linked_propositions = self.other_compare.dbquery.select_from_knowledge_people_by_uuid(ecs)
        linked_values = self.other_compare.values_prepare(linked_propositions)
        value_query = sentence.replace(subject, "", 1).strip()
        print(value_query)
        real_value, best_scores = self.other_compare.Sentencesimilarity(value_query, linked_values)

        # for value, score in zip(real_value, best_scores):
        #     print(f"相关键值: {value} | 相似度: {score:.4f}")

        if not real_value:
            return None, 0.0

        max_index = best_scores.index(max(best_scores))
        best_match = real_value[max_index]
        best_score = best_scores[max_index]
        return best_match, best_score
